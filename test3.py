...
excel_file = pd.ExcelFile('customer_network_analysis_20180603(1).xlsx')
...
df_shopcart = df_shopcart[df_shopcart['PAY_DATE'].dt.date == pd.to_datetime('2018-06-03').date()]
...
# 构建用户之间的社交关系图
def build_social_graph(df_shopcart):
    G = nx.Graph()
    # 添加节点
    for customer_id in df_shopcart['CUSTOMER_ID'].unique():
        G.add_node(customer_id)
    # 添加边，基于共同购买的产品
    for product_id, group in df_shopcart.groupby('PRODUCT_ID'):
        customer_ids = group['CUSTOMER_ID'].unique()
        for i in range(len(customer_ids)):
            for j in range(i + 1, len(customer_ids)):
                if G.has_edge(customer_ids[i], customer_ids[j]):
                    G[customer_ids[i]][customer_ids[j]]['weight'] += 1
                else:
                    G.add_edge(customer_ids[i], customer_ids[j], weight=1)
    return G
...
def collaborative_filtering_recommendation(target_customer_id, df_shopcart):
    # 构建顾客 - 产品矩阵
    customer_product_matrix = df_shopcart.pivot_table(index='CUSTOMER_ID', columns='PRODUCT_ID', aggfunc=lambda x: 1,
                                                      fill_value=0)
...
def menu_ingredient_recommendation(target_customer_id, df_shopcart, df_menu):
    ...
    customer_purchased_products = df_shopcart[
        ...
        (df_shopcart['PAY_DATE'].dt.date == pd.to_datetime('2018-06-03').date())
        ]['PRODUCT_NAME'].tolist()
...
# 社群推荐函数
def community_based_recommendation(target_customer_id, df_shopcart, partition):
    # 获取目标顾客所在的社群
    target_community = partition[target_customer_id]
    # 获取同一社群内的其他用户
    community_members = [customer_id for customer_id, community in partition.items() if
                         community == target_community and customer_id != target_customer_id]

    if not community_members:
        print("该顾客所在的社群中没有其他用户。")
        return []

    # 随机选择一个用户
    selected_customer_id = random.choice(community_members)
    # 获取该用户购买的所有产品
    recommended_products = df_shopcart[df_shopcart['CUSTOMER_ID'] == selected_customer_id]['PRODUCT_ID'].unique()
    # 将推荐的产品ID转换为产品名称
    recommended_product_names = [product_id_to_name.get(pid, "未知产品") for pid in recommended_products]
    return recommended_product_names


# 构建社交图并检测社群
G = build_social_graph(df_shopcart)
...
def recommend_for_customer(target_customer_id):
    ...
    # 获取顾客的节点中心性分类
    customer_domain = df_customer[df_customer['CUSTOMER_ID'] == target_customer_id]['总分类'].values[0]
    ...
    if customer_domain == '正域':
        ...
        print(
            f"为零界顾客 {target_customer_id} 协同过滤推荐的产品: {cf_recommended_products}，菜谱食材推荐的食材: {menu_recommended_ingredients}")
    elif customer_domain == '负域':
        cf_recommended_products = collaborative_filtering_recommendation(target_customer_id, df_shopcart)
        menu_recommended_ingredients = menu_ingredient_recommendation(target_customer_id, df_shopcart, df_menu)
        print(
            f"为负域顾客 {target_customer_id} 协同过滤推荐的产品: {cf_recommended_products}，菜谱食材推荐的食材: {menu_recommended_ingredients}")
    else:
        print(f"顾客ID {target_customer_id} 的分类 {customer_domain} 不在正域、零界或负域中。")

    # 社群推荐
    community_recommended_products = community_based_recommendation(target_customer_id, df_shopcart, partition)
    print(f"为顾客 {target_customer_id} 社群推荐的产品: {community_recommended_products}")
